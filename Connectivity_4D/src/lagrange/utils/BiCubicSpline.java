...
public class BiCubicSpline {
	...
	private double[][] yT = null; // transposed y=f(x1,x2) tabulated function,
									// i.e. y=f(x2,x1)
	private double[] x1 = null; // x1 in tabulated function f(x1,x2)
	private double[] x2 = null; // x2 in tabulated function f(x1,x2)
	private double xx1 = Double.NaN; // value of x1 at which an interpolated y
										// value is required
	private double xx2 = Double.NaN; // value of x2 at which an interpolated y
										// value is required
	...
	private double[][] d2ydx2innerT = null; // second derivatives of first
											// called transposed array of cubic
											// splines
	private CubicSpline csn[] = null; // nPoints array of CubicSpline instances
	private CubicSpline csm = null; // CubicSpline instance
	private CubicSpline csnT[] = null; // mPoints array of transposed
										// CubicSpline instances
	private CubicSpline csmT = null; // transposed CubicSpline instance
	private double interpolatedValue = Double.NaN; // interpolated value for the
													// original 2D matrix
	private double interpolatedValueTranspose = Double.NaN; // interpolated
															// value for the
															// transposed 2D
															// matrix
	private double interpolatedValueMean = Double.NaN; // mean interpolated
														// value
	...
	public BiCubicSpline(double[] x1, double[] x2, float[][] y) {
		this.nPoints = x1.length;
		this.mPoints = x2.length;
		this.nPointsT = this.mPoints;
		this.mPointsT = this.nPoints;
	...
	public BiCubicSpline(int nP, int mP) {
		...
		if (!this.roundingCheck)
			this.csm.noRoundingErrorCheck();
	...
	// Default option: points outside the interpolation bounds by less than the
	// potential rounding error rounded to the bounds limit
	// This method causes this check to be ignored and an exception to be thrown
	// if any point lies outside the interpolation bounds
	...
	// Default option: points outside the interpolation bounds by less than the
	// potential rounding error rounded to the bounds limit
	// The default value for the potential rounding error is 5e-15*times the
	// 10^exponent of the value outside the bounds
	...
	public void averageIdenticalAbscissae() {
		...
		for (int i = 0; i < this.csn.length; i++)
			this.csn[i].averageIdenticalAbscissae();
	...
	public void resetData(double[] x1, double[] x2, float[][] y) {
		if (x1.length != y.length)
			throw new IllegalArgumentException(
					"Arrays x1 and y row are of different length");
		if (x2.length != y[0].length)
	...
	public void resetData(float[] x1, float[] x2, float[][] y) {
		if (x1.length != y.length)
			throw new IllegalArgumentException(
					"Arrays x1 and y row are of different length");
		if (x2.length != y[0].length)
	...
	// Returns mean interpolated value, interpolated value for data as entered,
	// interpolated value for transposed matrix, xx1 value and xx2 value
	public double[] getInterpolatedValues() {
		double[] ret = { this.interpolatedValueMean, this.interpolatedValue,
				this.interpolatedValueTranspose, this.xx1, this.xx2 };
		return ret;
	}
	...
	/**
	 * Sets dependent values using a 2D double array
	 * 
	 * @param y
	 */
