 ...
 * 
 * DATE: May 2002 UPDATE: 20 May 2003, 17 February 2006, 27 July 2007, 4
 * December 2007, 31 October 2009, 5 January 2011
 * 
 ...
 * 
 * PERMISSION TO COPY: Permission to use, copy and modify this software and its
 * documentation for NON-COMMERCIAL purposes is granted, without fee, provided
 * that an acknowledgement to the author, Michael Thomas Flanagan at
 * http:\\www.ee.ucl.ac.uk/~mflanaga, appears in all copies.
 * 
...
public class TriCubicSpline {
	...
	private boolean averageIdenticalAbscissae = false; // if true: the the
														...
														// are averaged
														// If false: the
														// abscissae values are
														// separated by 0.001 of
														// the total abscissae
														// range;
	@SuppressWarnings("unused")
	 ...
	 * @param y
	 *            - 3-dimensional matrix of double data values.
	 */
	...
	public TriCubicSpline(double[] x1, double[] x2, double[] x3, double[][][] y) {
		...
		this.bcsn = BiCubicSpline.oneDarray(this.nPoints, this.mPoints,
				this.lPoints);
	...
	public TriCubicSpline(double[] x1, double[] x2, double[] x3, float[][][] y) {
		this.nPoints = x1.length;
		this.mPoints = x2.length;
		this.lPoints = x3.length;
		if (this.nPoints != y.length)
	...
	/**
	 * Constructor with data arrays initialised to zero Primarily for use by
	 * QuadriCubicSpline
	 * 
	 * @param nP
	 * @param mP
	...
	public TriCubicSpline(int nP, int mP, int lP) {
		...
		if (this.nPoints < 3 || this.mPoints < 3 || this.lPoints < 3)
			throw new IllegalArgumentException(
					"The data matrix must have a minimum size of 3 X 3 X 3");
	...
	/**
	 * Reset rounding error check option Default option: points outside the
	 * interpolation bounds by less than the potential rounding error rounded to
	 * the bounds limit This method causes this check to be ignored and an
	 * exception to be thrown if any point lies outside the interpolation bounds
	 */
	...
	/**
	 * Reset potential rounding error value Default option: points outside the
	 * interpolation bounds by less than the potential rounding error rounded to
	 * the bounds limit The default value for the potential rounding error is
	 * 5e-15*times the 10^exponent of the value outside the bounds This method
	 * allows the 5e-15 to be reset
	 * 
	 * @param potentialRoundingError
	 */
	 ...
	 * of the range to averaging the relevant ordinates
	 */

	@SuppressWarnings("static-access")
	public void averageIdenticalAbscissae() {
		this.averageIdenticalAbscissae = true;
		for (int i = 0; i < this.bcsn.length; i++)
	 ...
	 * and all array values to zero with natural spline default Primarily for
	 * use in this.oneDarray for QuadriCubicSpline
	 * 
	 * @param nP
	 * @param mP
	 * @param lP
	...
	public static TriCubicSpline zero(int nP, int mP, int lP) {
		if (nP < 3 || mP < 3 || lP < 3)
			throw new IllegalArgumentException(
					"A minimum of three x three x three data points is needed");
		TriCubicSpline aa = new TriCubicSpline(nP, mP, lP);
		return aa;
	}

	/**
	 * Create a one dimensional array of TriCubicSpline objects of length nP
	 * each of internal array size mP x lP xkP Primarily for use in
	 * quadriCubicSpline
	 * 
	 * @param nP
	 * @param mP
	...
	public double[] getLimits() {
		double[] limits = { xMin[0], xMax[0], xMin[1], xMax[1], xMin[2],
				xMax[2] };
		return limits;
	}
	...
	/**
	 * Gets the inner matrix of derivatives Primarily used by QuadriCubicSpline
	 */
	...
	/**
	 * Sets the inner matrix of derivatives Primarily used by QuadriCubicSpline
	 */
	 ...
	 * 
	 * @param y
	 *            - a 3-dimensional matrix of double values.
	 */

	public void setValues(double[][][] y) {
		this.y = y;
		for (int i = 0; i < this.nPoints; i++) {
			bcsn[i].setValues(y[i]);
		}
