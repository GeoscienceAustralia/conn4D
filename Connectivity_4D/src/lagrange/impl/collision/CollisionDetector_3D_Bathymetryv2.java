...
public class CollisionDetector_3D_Bathymetryv2 implements CollisionDetector {

	private Boundary_NetCDF_Grid bnd;
	private Intersector_3D_Raster i3d = new Intersector_3D_Raster();

	public CollisionDetector_3D_Bathymetryv2(Boundary bathym) {
	...
	public void handleIntersection(Particle p) {
		...
		// Keep the particle beneath sea level
		if(trans.p1.z>0){trans.p1.z = 0;}
		...
		while (!Arrays.equals(currentCell, endCell)
			...
			if (dir[0] == 1) {
				if (currentCell[0] > Math.max(startCell[0], endCell[0])) {
					System.out.println("WTF2?");
					handleIntersection(p);
				}
			}
			if (dir[1] == -1) {
				if (currentCell[1] < Math.min(startCell[1], endCell[1])) {
					System.out.println("WTF3?");
					handleIntersection(p);
				}
			}
			if (dir[1] == 1) {
				if (currentCell[1] > Math.max(startCell[1], endCell[1])) {
			...
			//trans = new LineSegment(CoordinateMath.lonlat2ceqd(backtrans.p0),CoordinateMath.lonlat2ceqd(backtrans.p1));
			trans = i3d.reflect(trans, box);
			backtrans = new LineSegment(CoordinateMath.ceqd2lonlat(trans.p0),CoordinateMath.ceqd2lonlat(trans.p1));
		}
